...
describe('useHarmonyStore', () => {
  ...
  it('changes time signature and resizes patterns', () => {
    const { addChord } = useHarmonyStore.getState();
    addChord({ name: 'Am', degree: 'vi', key: 'C', mode: 'major' });
    expect(useHarmonyStore.getState().current.chords[0].strumPattern).toHaveLength(8);

    useHarmonyStore.getState().setTimeSignature('3/4');
    expect(useHarmonyStore.getState().current.timeSignature).toBe('3/4');
    expect(useHarmonyStore.getState().current.chords[0].strumPattern).toHaveLength(6);
  });

  it('saves and loads a harmony', () => {
    const { addChord, setName, saveHarmony, clearCurrent, loadHarmony } = useHarmonyStore.getState();
    addChord({ name: 'C', degree: 'I', key: 'C', mode: 'major' });
  ...
  it('deletes a saved harmony', () => {
    const { addChord, setName, saveHarmony, deleteHarmony } = useHarmonyStore.getState();
    addChord({ name: 'C', degree: 'I', key: 'C', mode: 'major' });
    setName('To Delete');
    saveHarmony();
    const savedId = useHarmonyStore.getState().saved[0].id;
